...
LABEL name="Black Duck OpsSight Pod Processor" \
      ...
      description="This container is used to identify all images inside each pod in a Kubernetes or Red Hat OpenShift cluster. It will send all the identified images to opssight-core for scanning. It will also retrieve vulnerability and policy violations for each image periodically from opssight-core and annotate and label them in all pods." \
