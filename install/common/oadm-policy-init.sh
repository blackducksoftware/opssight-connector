...
route_docker_registry=$(oc get route docker-registry -o jsonpath='{.spec.host}')
service_docker_registry=$(oc get svc docker-registry -o jsonpath='{.spec.clusterIP}')
service_docker_registry_port=$(oc get svc docker-registry -o jsonpath='{.spec.ports[0].port}')
